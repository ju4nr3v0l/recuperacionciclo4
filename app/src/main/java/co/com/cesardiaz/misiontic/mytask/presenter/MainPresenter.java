...
public class MainPresenter implements MainMVP.Presenter {
    ...
    public void addNewTask() {
        Log.i(MainPresenter.class.getSimpleName(), "Crear nueva tarea");
    ...
    public void taskItemClicked(TaskItem task) {
        String message = task.getState() == TaskState.PENDING
                ? "Quiere marcar la tarea como HECHA?"
                : "Quiere marcar la tarea como PENDIENTE?";
    ...
    public void taskItemLongClicked(TaskItem task) {
        if (task.getState() == TaskState.DONE) {
            view.showDeleteDialog("Quiere ELIMINAR esta tarea?", task);
        }
    }
